...
class WhereCommand : TabExecutor {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        val component =
            TextComponent("§c${player.location.x.toInt()} ${player.location.y.toInt()} ${player.location.z.toInt()}")
        component.isUnderlined = true
        component.hoverEvent =
            HoverEvent(HoverEvent.Action.SHOW_TEXT, arrayOf(TextComponent("§aClick to copy Coordinates")))
        component.clickEvent = ClickEvent(
            ClickEvent.Action.COPY_TO_CLIPBOARD,
            "${player.location.x.toInt()} ${player.location.y.toInt()} ${player.location.z.toInt()}"
        )
        Bukkit.getServer().spigot()
            .broadcast(TextComponent("${ModionePlugin.PREFIX}§6${player.name}§a is at "), component)
        return true
    }
